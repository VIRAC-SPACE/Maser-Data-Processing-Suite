...
class Analyzer(QWidget):
    ...
    def __init__(self, source, line, iteration_number, log_file):
        ...
        if not os.path.exists( get_configs( "paths", "logPath" ) + "SDR/" + self.log_file ):
            print( "aaa", get_configs( "paths", "logPath" ) + "SDR/" + self.log_file )
            # self.close()
            # self.destroy()
    ...
    def plot_total_results(self):
        ...
        for s in range( 0, len( y__left_avg ) ):
            ...
            # tmp_output_file_name = self.source + "_" +self.iteration_number + "_" + str(s) + ".txt"
