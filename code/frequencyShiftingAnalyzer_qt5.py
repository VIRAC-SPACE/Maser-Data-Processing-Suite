...
class Analyzer(QWidget):
        ...
        if filter > 0:
            ...
            for x in range(filter):
                ...
                if x == filter - 1:
                    ...
                    async_result1 = pool.apply_async(replaceBadPoints, (
                    xdata, ydata_1_u1, self.x_bad_point_1_u1, self.y_bad_point_1_u1, data1))
                    async_result2 = pool.apply_async(replaceBadPoints, (
                    xdata, ydata_1_u9, self.x_bad_point_1_u9, self.y_bad_point_1_u9, data1))
                    async_result3 = pool.apply_async(replaceBadPoints, (
                    xdata, ydata_2_u1, self.x_bad_point_2_u1, self.y_bad_point_2_u1, data2))
                    async_result4 = pool.apply_async(replaceBadPoints, (
                    xdata, ydata_2_u9, self.x_bad_point_2_u9, self.y_bad_point_2_u9, data2))
    ...
    def __PAIR(self, index, totalResults_u1, totalResults_u9, STON_list_u1, STON_list_u9, STON_list_AVG):
        ...
        try:
            self.data_1 = np.fromfile(scanNUmber1, dtype="float64", count=-1, sep=" ").reshape(
                (file_len(scanNUmber1), 9))
            self.data_2 = np.fromfile(scanNUmber2, dtype="float64", count=-1, sep=" ").reshape(
                (file_len(scanNUmber2), 9))
        except IOError as e:
            print("IO Error", e)
            sys.exit(1)
    ...
    def plotTotalResults(self):
        ...
        for p in range(0, self.datPairsCount):
            ...
            t = datetime.datetime(int(dateStr[0]), int(dateStr[1]), int(dateStr[2]), int(timeStr[0]), int(timeStr[1]),
                                  int(timeStr[2]))
            time = t.isoformat()
